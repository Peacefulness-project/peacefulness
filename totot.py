...
pathExport = "Results/toto" + str(DSM_proportion) + "_" + strategy_exchange + "_" + strategy_distribution + "_" + grid  # directory where results are written
...
if grid == "mixed":
    ...
    if strategy_exchange != "BAU": # in cases with DSM, the power of the HP is considered "infinite" and equal to 10 MW otherwise
        aggregator_heat = Aggregator(aggregator_name, LTH, supervisor_heat, DHN_manager, aggregator_elec, contract_DHN, 2.5, {"buying": 200, "selling": 0})  # creation of a aggregator
    else:
        aggregator_heat = Aggregator(aggregator_name, LTH, supervisor_heat, DHN_manager, aggregator_elec, contract_DHN, 2.5, {"buying": 200, "selling": 0})  # creation of a aggregator
...
if grid == "mixed":
    subclasses_dictionary["Device"]["DummyProducer"]("heat_production", contract_flexible_production_heat, producer, aggregator_heat, {"device": "heat"}, {"max_power": 710})  # creation of a heat production unit
elif grid == "elec":
