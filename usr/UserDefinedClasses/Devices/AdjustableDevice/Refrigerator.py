            ...
            if energy_wanted_min:  # if the device is active
                ...
                if not (energy_wanted_min < energy_accorded < energy_wanted_max):  # if the energy given is not in the borders defined by the user
                    ...
                    for nature in self.natures:
                        ...
                        dissatisfaction = self.natures[nature][1].dissatisfaction_modification(dissatisfaction)  # here, the contract may modify dissatisfaction                        self._catalog.set(f"{self.agent.name}.dissatisfaction", dissatisfaction)
...
user_classes_dictionary[f"{Refrigerator.__name__}"] = Refrigerator
