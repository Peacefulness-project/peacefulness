...
class Grid(Supervisor):
    ...
    def distribute_remote_energy(self, cluster):  # the grid, as it has no devices, has no distribution to make
        ...
        for managed_cluster in cluster.subclusters:
            quantities = self._catalog.get(f"{managed_cluster.name}.{managed_cluster.nature.name}.quantities_asked")
            ...
            self._catalog.set(f"{managed_cluster.name}.{managed_cluster.nature.name}.quantities_given", quantities)
