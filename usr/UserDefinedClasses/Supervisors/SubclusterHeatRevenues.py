...
class SubclusterHeatRevenues(Supervisor):
    ...
    def distribute_remote_energy(self, cluster):  # after having exchanged with the exterior, the cluster
        ...
        while energy_available_consumption > sorted_demands[i][1] and i < len(
            ...
            try:  # if it is a subcluster
                ...
                Emin = self._catalog.get(f"{device_name}.{cluster.nature.name}.energy_accorded")[
                    "quantity"]  # we get back the minimum, which has already been served

                self._catalog.set(f"{device_name}.{cluster.nature.name}.energy_accorded",
                                  {"quantity": Emin + energy, "price": price})
        ...
        while energy_available_production > - sorted_offers[i][1] and i < len(
            ...
            try:  # if it is a subcluster
                quantities_given = self._catalog.get(f"{device_name}.quantities_asked")
                quantities_given.append([-energy, price])
        ...
        if sorted_offers[i][1]:  # if the demand really exists
            device_name = sorted_offers[i][3]
            # print(energy_available_production)
            energy = min(-sorted_offers[i][1], energy_available_production)  # the quantity of energy needed
            price = sorted_offers[i][2]  # the price of energy
            price = max(price, min_price)
...
user_classes_dictionary[f"{SubclusterHeatRevenues.__name__}"] = SubclusterHeatRevenues
