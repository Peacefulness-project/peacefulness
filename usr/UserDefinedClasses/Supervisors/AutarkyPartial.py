...
class AutarkyPartial(Supervisor):
    ...
    def distribute_remote_energy(self, cluster):  # after having exchanged with the exterior, the cluster
        ...
        if maximum_energy_produced <= minimum_energy_consumed or maximum_energy_consumed <= minimum_energy_produced:  # if there is no possibility to balance the grid
            ...
            [maximum_energy_produced, money_earned_inside, energy_sold_inside] = self._distribute_consumption_partial_service(cluster, max_price, sorted_demands, maximum_energy_produced, money_earned_inside, energy_sold_inside)
...
user_classes_dictionary[f"{AutarkyPartial.__name__}"] = AutarkyPartial
