...
class AutarkyEmergency(Supervisor):
    ...
    def distribute_remote_energy(self, cluster):  # after having exchanged with the exterior, the cluster
        ...
        if maximum_energy_produced <= minimum_energy_consumed or maximum_energy_consumed <= minimum_energy_produced:  # if there is no possibility to balance the grid
            ...
            [sorted_demands, sorted_offers] = self._emergency_sort(cluster)  # sort the quantities by decreasing emergency
            ...
            if maximum_energy_consumed >= maximum_energy_produced:  # if there is not enough production to satisfy all the consumption
                ...
                try:
                    ...
                    self._catalog.set(f"{device_name}.{cluster.nature.name}.energy_accorded", {"quantity": Emin + energy_available, "price": price})
        ...
        # ##########################################################################################
            ...
            else:  # if there is not enough consumption to absorb all the offer
...
user_classes_dictionary[f"{AutarkyEmergency.__name__}"] = AutarkyEmergency
