...
class WhenProfitable(Supervisor):
    ...
    def ascendant_phase(self, cluster):  # before communicating with the exterior, the cluster makes its local balances
        ...
        for i in range(len(sorted_demands)):  # demands
            if sorted_demands[i][0] == 1:  # if it is urgent
                ...
                try:  # if it is a subcluster
                    ...
                    self._catalog.set(f"{sorted_demands[i][3]}.{cluster.nature.name}.energy_accorded",
                                      {"quantity": sorted_demands[i][1], "price": sorted_demands[i][2]})

                money_earned_inside += sorted_demands[i][1] * sorted_demands[i][
                    2]  # money earned by selling energy to the device
            ...
            if sorted_offers[i][0] == 1:  # if it is urgent
                ...
                try:  # if it is a subcluster
                    ...
                    self._catalog.set(f"{device_name}.{cluster.nature.name}.energy_accorded",
                                      {"quantity": Emax, "price": price})

                money_spent_inside -= sorted_offers[i][1] * sorted_offers[i][
                    2]  # money spent by buying energy from the subcluster
        ...
        quantities_and_prices = [[energy_difference, sign(
            energy_difference) * inf]]  # wants to satisfy everyone, regardless the price (i.e sells even at -inf and buys even at +inf)
        ...
        while buying_price >= selling_price and i < len(
                sorted_demands) - 1:  # as long the buying price is aboce the selling one and that there is demand
            demand += sorted_demands[i][1]
            buying_price = sorted_demands[i][2]
            final_price = buying_price

            while buying_price >= selling_price and offer <= demand and j < len(
                    sorted_offers) - 1:  # as long as there is offer
        ...
        quantities_exchanged = min(offer,
                                   demand)  # the quantity exchanged internally is the minimum of the current offer and demand
        self._quantities_exchanged_internally[cluster.name] = [quantities_exchanged,
                                                               final_price]  # we store this value for the descendant phase
        ...
        price_rest_demand = (final_price + sorted_demands[-1][
            2]) / 2  # the price used is the mean between the intern price and the lowest price

        quantities_and_prices.append(
            [quantity_rest_demand, price_rest_demand])  # the quantity the cluster wants to buy cheaply
        ...
        price_rest_offer = (final_price + sorted_offers[-1][
            2]) / 2  # the price used is the mean between the intern price and the highest price

        quantities_and_prices.append(
            [quantity_rest_offer, price_rest_offer])  # the quantity the cluster wants to sell expensively
    ...
    def distribute_remote_energy(self, cluster):  # after having exchanged with the exterior, the cluster
        ...
        energy_bought_inside = self._catalog.get(f"{cluster.name}.energy_bought")[
            "inside"]  # the absolute value of energy bought inside
        energy_sold_inside = self._catalog.get(f"{cluster.name}.energy_sold")[
            "inside"]  # the absolute value of energy sold inside
        ...
        money_earned_inside = self._catalog.get(f"{cluster.name}.money_earned")[
            "inside"]  # the absolute value of money earned inside
        money_spent_inside = self._catalog.get(f"{cluster.name}.money_spent")[
            "inside"]  # the absolute value of money spent inside
        ...
        energy_available_consumption = self._quantities_exchanged_internally[cluster.name][
                                           0] + energy_bought_outside  # the total energy available for consumptions
        energy_available_production = self._quantities_exchanged_internally[cluster.name][
                                          0] + energy_sold_outside  # the total energy available for productions
        ...
        i = 0
        while energy_available_consumption > sorted_demands[i][1] and i < len(
                sorted_demands) - 1:  # as long as there is energy available
