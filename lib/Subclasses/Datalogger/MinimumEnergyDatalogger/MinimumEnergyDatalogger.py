...
class MinimumEnergyDatalogger(Datalogger):  # a subclass of datalogger designed to export the Emin of the devices
    ...
    def __init__(self, name: str, filename: str, devices_list: List[str], period=1, graph_options="default"):
        ...
        super().__init__(name, filename, period, graph_options=graph_options, graph_labels={"xlabel": "time", "ylabel": f"Emin"})

        self.add(f"simulation_time", graph_status="X")
        self.add(f"physical_time", graph_status="")

        def create_get_Emin_function(device_name, nature_name):

            def get_Emin(name):
                Emin = self._catalog.get(f"{device_name}.{nature_name}.energy_wanted")["energy_minimum"]
        ...
        for device_name in self._devices_list.keys():  # for each nature registered into world, the Emin key is added
            for nature_name in self._devices_list[device_name][0]:
                aggregator_name = self._devices_list[device_name][1][0].name
                self.add(f"{device_name}.{aggregator_name}.{nature_name}.minimum_energy", create_get_Emin_function(device_name, nature_name), graph_status="Y")
