...
class DeviceSubclassBalancesDatalogger(Datalogger):  # a sub-class of dataloggers designed to export the balances
    ...
    def __init__(self, subclass, period=1):
        self._devices_list = {}

        if period == "global":
            super().__init__(f"device_{subclass}_balances_global", f"DeviceBalances_{subclass}_global", period)
        else:
            super().__init__(f"device_{subclass}_balances_frequency_{period}", f"DeviceBalances_{subclass}_frequency_{period}", period)
        ...
        self.add(f"{subclass}.energy", get_quantity)
        self.add(f"{subclass}.money", get_money)
