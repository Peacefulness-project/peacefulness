    ...
    def __init__(self, name: str, filename: str, devices_list: List[str], period=1, graph_options="default"):
        ...
        for device_name in self._catalog.get("dictionaries")['devices'].keys():
            if self._catalog.get("dictionaries")['devices'][device_name].name in devices_list:
                self._devices_list[device_name] = {"natures" : [nature.name for nature in self._catalog.get("dictionaries")['devices'][device_name].natures],
                                                   "aggregators": self._catalog.get("dictionaries")['devices'][device_name].device_aggregators_names,
                                                   "type": self._catalog.get("dictionaries")['devices'][device_name].get_type}  # get all the names
        ...
        for device_name in self._devices_list.keys():  # for each nature registered into world, all the relevant keys are added
            for nature_name in self._devices_list[device_name]["natures"]:
                self.add(f"{self._devices_list[device_name]["type"]}.{device_name}.{self._devices_list[device_name]["aggregators"]}.energy", create_get_quantity_function(device_name, nature_name), graph_status="Y")
                self.add(f"{self._devices_list[device_name]["type"]}.{device_name}.{self._devices_list[device_name]["aggregators"]}.money", create_get_price_function(device_name, nature_name), graph_status="Y2")
