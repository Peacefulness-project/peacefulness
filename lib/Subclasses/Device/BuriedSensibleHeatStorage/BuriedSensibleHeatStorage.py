...
class BuriedSensibleHeatStorage(Storage):

    def __init__(self, name, contracts, agent, aggregator, profiles, parameters, filename="lib/Subclasses/Device/BuriedSensibleHeatStorage/BuriedSensibleHeatStorage.json"):
        ...
        self._location = ground_temperature_daemon.location  # the location of the device, in relation with the meteorological data
    ...
    def _degradation_of_energy_stored(self):  # a class-specific function reducing the energy stored over time
        ...
        storage_temperature = storage_temperature + (self._thermal_conductivity * (ground_temperature - storage_temperature) * self._surface * time_step)/(self._thickness * self._density * self._volume * self._thermal_capacity)  # calculation of the loss of temperature
