...
class DomesticHeatPumpReversible(AdjustableDevice):

    def __init__(self, name, contracts, agent, aggregators, profiles, parameters, filename="lib\Subclasses\Device\DomesticHeatPumpReversible\DomesticHeatPumpReversible.json"):
    ...
    def update(self):  # method updating needs of the devices before the supervision
        ...
        for line in self._user_profile:
            ...
            if line[0] == self._moment:  # if a consumption has been scheduled and if it has not been fulfilled yet
                ...
                print(Tcs, Tce, Tfe, Tfs)

                Tmc = (Tcs - Tce) / (log(Tcs / Tce))
                Tmf = (Tfs - Tfe) / (log(Tfs / Tfe))

                if current_indoor_temperature < line[2][0]:
                    COP = (Tmc - 273.15) / (Tmc - Tmf)
                elif current_indoor_temperature > line[2][2]:
                    COP = (Tmf - 273.15) / (Tmf - Tmc)
                else:
                    COP = 3
