# This subclass of device represent undeground geothermal storage
# Current packages
...
class UndergroundThermalStorage(Storage):
    ...
    def _read_data_profiles(self, profile):
        ...
        self._network_temperature = data_device["network_temperature"]  # °C, TODO: le remplacer par un démon ?
        ...
        self._capacity = self._max_energy - self._min_energy  # energy_corresponding to the maximum usable energy
        self._catalog.add(f"{self.name}.energy_stored", self._temperature_to_energy(self._initial_storage_temperature + 273.15) - self._min_energy)  # the energy stored at a given time, considered as half charged at the beginning
    ...
    def _degradation_of_energy_stored(self):  # a class-specific function reducing the energy stored over time
        ...
        storage_temperature = self._energy_to_temperature(energy_stored + self._min_energy)  # conversion of stored energy to temperature of the storage
        ground_temperature = self._catalog.get(f"{self._location}.ground_temperature")
        time_step = self._catalog.get("time_step")

        storage_temperature = storage_temperature - (self._thermal_conductivity * (storage_temperature - ground_temperature - 273.15) * self._heat_loss_factor * self._storage_depth / 2 * time_step) / (self._volume * self._thermal_capacity)  # calculation of the loss of temperature
        # storage temperature is considered constant in the heat transfer calculation
        return self._temperature_to_energy(storage_temperature) - self._min_energy  # actualisation of the energy stored
    ...
    def _discharging_power(self):
        ...
        storage_temperature = self._energy_to_temperature(energy_stored + self._min_energy)  # conversion of stored energy to temperature of the storage
        time_step = self._catalog.get("time_step")
        energy_flow = max(0, self._exchanger_conductance * (storage_temperature - self._network_temperature - 273.15) * time_step)
        return energy_flow
