...
class GymStrategy(Strategy):

    def __init__(self, optional_params=None):
        super().__init__("gym_strategy", "The strategy that will be learned by the RL agent with the Gym environment")

        # To apply the action of the RL agent
        self._catalog.add(f"gym_Strategy.decision_message", {})
        self._catalog.add(f"gym_Strategy.exchanges_message", {})
        ...
        if optional_params:
            ...
            if f"gym_Strategy.sorting_coefficients" not in self._catalog.keys:
                self._catalog.add(f"gym_Strategy.sorting_coefficients", optional_params)
            else:
                self._catalog.set(f"gym_Strategy.sorting_coefficients", optional_params)
        else:
    ...
    def bottom_up_phase(self, aggregator: "Aggregator"):
        ...
        if f"{aggregator.name}.gym_Strategy.energy_prices" not in self._catalog.keys:
            self._catalog.add(f"{aggregator.name}.gym_Strategy.energy_prices", {"buying_price": deepcopy(buying_price), "selling_price": deepcopy(selling_price)})
        else:
            self._catalog.set(f"{aggregator.name}.gym_Strategy.energy_prices", {"buying_price": deepcopy(buying_price), "selling_price": deepcopy(selling_price)})
        ...
        if forecasting_message is not None:
            if f"{aggregator.name}.gym_Strategy.forecasting_message" not in self._catalog.keys:
                self._catalog.add(f"{aggregator.name}.gym_Strategy.forecasting_message", deepcopy(forecasting_message))
            else:
                self._catalog.set(f"{aggregator.name}.gym_Strategy.forecasting_message", deepcopy(forecasting_message))
        ...
        if f"{aggregator.name}.gym_Strategy.converter_message" not in self._catalog.keys:
            self._catalog.add(f"{aggregator.name}.gym_Strategy.converter_message", deepcopy(converter_message[aggregator.name]))
        else:
            self._catalog.set(f"{aggregator.name}.gym_Strategy.converter_message", deepcopy(converter_message[aggregator.name]))
        ...
        if f"{aggregator.name}.gym_Strategy.energy_maximum_quantities" not in self._catalog.keys:
            self._catalog.add(f"{aggregator.name}.gym_Strategy.energy_maximum_quantities", {"consumption": max_conso, "production": max_prod})
        else:
            self._catalog.set(f"{aggregator.name}.gym_Strategy.energy_maximum_quantities", {"consumption": max_conso, "production": max_prod})
        if f"{aggregator.name}.gym_Strategy.energy_minimum_quantities" not in self._catalog.keys:
            self._catalog.add(f"{aggregator.name}.gym_Strategy.energy_minimum_quantities", {"consumption": deepcopy(minimum_energy_consumed), "production": deepcopy(minimum_energy_produced)})
        else:
            self._catalog.set(f"{aggregator.name}.gym_Strategy.energy_minimum_quantities", {"consumption": deepcopy(minimum_energy_consumed), "production": deepcopy(minimum_energy_produced)})

        if f"{aggregator.name}.gym_Strategy.formalism_message" not in self._catalog.keys:
            self._catalog.add(f"{aggregator.name}.gym_Strategy.formalism_message", deepcopy(formalism_message))
        else:
            self._catalog.set(f"{aggregator.name}.gym_Strategy.formalism_message", deepcopy(formalism_message))
        ...
        if direct_exchanges:
            if aggregator.name in direct_exchanges:
                if f"{aggregator.name}.gym_Strategy.direct_energy_exchanges" not in self._catalog.keys:
                    self._catalog.add(f"{aggregator.name}.gym_Strategy.direct_energy_exchanges", deepcopy(direct_exchanges[aggregator.name]))
                else:
                    self._catalog.set(f"{aggregator.name}.gym_Strategy.direct_energy_exchanges", deepcopy(direct_exchanges[aggregator.name]))
            else:
                if f"{aggregator.name}.gym_Strategy.direct_energy_exchanges" not in self._catalog.keys:
                    self._catalog.add(f"{aggregator.name}.gym_Strategy.direct_energy_exchanges", deepcopy(direct_exchanges[aggregator.superior.name]))
                else:
                    self._catalog.set(f"{aggregator.name}.gym_Strategy.direct_energy_exchanges", deepcopy(direct_exchanges[aggregator.superior.name]))
    ...
    def top_down_phase(self, aggregator: "Aggregator"):  # todo à revoir dès avoir Gym + SB3 et PettingZoo + RLRay stable
        ...
        [energy_accorded_to_consumers, energy_accorded_to_producers, energy_accorded_to_storage] = implement_my_interior_decision(self._catalog, aggregator)
        energy_accorded_to_exchange = implement_my_exchange_decision(self._catalog, aggregator)

        if f"{aggregator.name}.gym_Strategy.internal_decision" not in self._catalog.keys:
            self._catalog.add(f"{aggregator.name}.gym_Strategy.internal_decision", [energy_accorded_to_consumers, energy_accorded_to_producers, energy_accorded_to_storage])
        else:
            self._catalog.set(f"{aggregator.name}.gym_Strategy.internal_decision", [energy_accorded_to_consumers, energy_accorded_to_producers, energy_accorded_to_storage])

        if f"{aggregator.name}.gym_Strategy.exchange_decision" not in self._catalog.keys:
            self._catalog.add(f"{aggregator.name}.gym_Strategy.exchange_decision", energy_accorded_to_exchange)
        else:
            self._catalog.set(f"{aggregator.name}.gym_Strategy.exchange_decision", energy_accorded_to_exchange)

        internal_buying_price, internal_selling_price = self._catalog.get(f"{aggregator.name}.gym_Strategy.energy_prices").values()
        maximum_energy_consumed, maximum_energy_produced = self._catalog.get(f"{aggregator.name}.gym_Strategy.energy_maximum_quantities").values()
        ...
        if aggregator.superior not in self._catalog.get(f"gym_Strategy.strategy_scope"):
