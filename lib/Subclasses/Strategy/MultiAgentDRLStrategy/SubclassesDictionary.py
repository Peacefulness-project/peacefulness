...
from lib.Subclasses.Strategy.SingleAgentDRLStrategy.Utilities import correct_path, modify_path, truncate_left

def get_subclasses():  # this function returns all subclasses
    subclasses_dictionary = {}  # a dictionary containing all the subclasses
    abs_path = Path(__file__).resolve().parents[4]
    working_path = modify_path(str(abs_path)) + "/" + "lib/Subclasses/"
    classes_concerned = list_files_and_folders(working_path)  # the list of all classes concerned by subclasses
    for class_name in classes_concerned:
        ...
        class_directory = working_path + class_name  # the directory containing subclasses of one class
        ...
        for subclass_folder in subclasses_list:  # for each subclass
            subclass_file = correct_path(working_path) + class_name + "." + subclass_folder + "." + subclass_folder  # the file where the module is defined
            if subclass_folder + ".py" in list_files_and_folders(working_path + class_name + "/" + subclass_folder):
                subclass_module = import_module(truncate_left(subclass_file, "lib"))  # we import the .py file
                try:
                    subclass_module = import_module(truncate_left(subclass_file, "lib"))  # we import the .py file
                except:
                    raise SubclassesInstantiationException(f"An error occured during the instantiation of the module {truncate_left(subclass_file, "lib")}.\n"
