...
class HeatPump(Converter):
    ...
    def second_update(self):
        energy_nominal = - self._catalog.get(f"{self.name}.{self.natures['downstream'].name}.energy_asked")["quantity"] / self._efficiency  # the energy asked by the downstream aggregator to the upstream one
    ...
    def first_react(self):  # this method adapts the converter to the decision taken by the upstream aggregator
        ...
        price = self._catalog.get(f"{self.name}.{self.natures['downstream'].name}.energy_asked")["price"]  # the price agreed with the downstream aggregator

        self._catalog.set(f"{self.name}.{self.natures['downstream'].name}.energy_accorded", {"quantity": energy_accorded, "price": price})  # the quantity of energy furnished to the downstream aggregator
