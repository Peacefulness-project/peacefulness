# This strategy is designed to test the request method fo dataloggers.
# It is similar to AlwaysSatisfied except that is sends requests at the beginning of the bottom-up phase.
...
class DataLoggerRequestsStrategy(Strategy):

    def __init__(self, datalogger_name: str, key_list: List[str]):
        super().__init__("always_satisfied_strategy", "Always serves everybody, whatever it can cost to him.")
        self._datalogger_name = datalogger_name
        self._key_list = key_list  # the keys to be requested
