...
pathExport = "cases/Studies/LOCIE_1/FigureRuns/Results/ProdOn1day/BAUmean"   # directory where results are written
...
start_date = datetime(year=2019, month=6, day=1, hour=0, minute=0, second=0, microsecond=0)
world.set_time(start_date,  # time management: start date
               1,  # value of a time step (in hours)
               24 * 2)  # number of time steps simulated
...
subclasses_dictionary["Device"]["Photovoltaics"]("PV_field", contract_elec, PV_producer, aggregator_elec, {"device": "standard_field"}, {"panels": 8617, "irradiation_daemon": irradiation_daemon.name, "outdoor_temperature_daemon": outdoor_temperature_daemon.name})  # creation of a photovoltaic panel field

subclasses_dictionary["Device"]["SolarThermalCollector"]("solar_thermal_collector_field", contract_heat, solar_thermal_producer, aggregator_heat, {"device": "standard_field"}, {"panels": 4016, "irradiation_daemon": irradiation_daemon.name, "outdoor_temperature_daemon": outdoor_temperature_daemon.name})  # creation of a solar thermal collector
...
def get_quantity(name):
    return abs(world.catalog.get(name)["quantity"])


def get_HP_consumption(name):
    return world.catalog.get(name)["outside"]/3.6


def get_energy_bought_outside(name):
    return world.catalog.get(name)["outside"]


def get_energy_sold_outside(name):
    return world.catalog.get(name)["outside"]


# repartition electricity
datalogger_electricity = Datalogger("elec_repartition", "ElectricityProduction", graph_options=graph_options, graph_labels={"xlabel": "time (\\textit{h})", "ylabel": "energy consumption or production (\\textit{kWh})"})

datalogger_electricity.add("simulation_time", graph_status="X")

datalogger_electricity.add("LVE.energy_consumed", graph_legend="electricity final consumption")
datalogger_electricity.add("PV_field.LVE.energy_accorded", function=get_quantity, graph_legend="PV production")
datalogger_electricity.add("Local_DHN.energy_bought", function=get_HP_consumption, graph_legend="heat pump consumption")
datalogger_electricity.add("general_aggregator.energy_bought", function=get_energy_bought_outside, graph_legend="electricity bought from national grid")
datalogger_electricity.add("general_aggregator.energy_sold", function=get_energy_sold_outside, graph_legend="electricity sold to national grid")


# repartition heat
datalogger_heat = Datalogger("heat_repartition", "HeatProduction", graph_options=graph_options, graph_labels={"xlabel": "time (\\textit{h})", "ylabel": "energy consumption or production (\\textit{kWh})"})

datalogger_heat.add("simulation_time", graph_status="X")
datalogger_heat.add("LTH.energy_consumed", graph_legend="heat consumption")
datalogger_heat.add("solar_thermal_collector_field.LTH.energy_accorded", function=get_quantity, graph_legend="solar thermal collectors production")
datalogger_heat.add("Local_DHN.energy_bought", function=get_energy_bought_outside, graph_legend="heat pump production")
