...
def assess_storage(strategy: "Strategy", aggregator: "Aggregator", demands: List[Dict]) -> float:
    ...
    for demand in demands:
        if demand["name"] == "thermal_storage":
...
# HP
def assess_HP(strategy: "Strategy", aggregator: "Aggregator", offers: List[Dict]) -> float:
    quantity_for_this_option = 0

    for demand in offers:
        if demand["name"] == "HP_unit":
            quantity_for_this_option -= demand["quantity"] - demand["quantity_min"]

    return quantity_for_this_option


def exchanges_HP(strategy: "Strategy", aggregator: "Aggregator", quantity_to_affect: float, quantities_and_prices: List[Dict]) -> Tuple:
    # quantity_to_affect = max(0, quantity_to_affect - quantity_available_for_this_option)
    return quantity_to_affect, quantities_and_prices


def distribution_HP(strategy: "Strategy", aggregator: "Aggregator", min_price: float, sorted_offers: List[Dict], energy_available_production: float, money_spent_inside: float, energy_bought_inside: float):
    i = 0
    name = "HP_unit"
    while sorted_offers[i]["name"] != name:  # as long as the storage is not found
        i += 1
...
index = ["CHP", "HP", "unstorage"]
columns = ["assess", "exchange", "distribute"]
data = [[assess_CHP, exchanges_CHP, distribution_CHP],
        [assess_HP, exchanges_HP, distribution_HP],
