...
strategy_elec = User.Strategies.AlwaysSatisfied.AlwaysSatisfied(name_strategy, description)
world.register_strategy(strategy_elec)

# the heat strategy
description = "Always serves everybody, whatever it can cost to him."
name_strategy = "heat_strategy"
strategy_heat = User.Strategies.SubaggregatorHeatEmergency.SubaggregatorHeatEmergency(name_strategy, description)
...
grid_strategy = User.Strategies.Grid.Grid(name_strategy, description)
...
# producers
# as we consider that they are the same as the strategy, we chose to put all their prices to 0
TOU_prices = "TOU_prices"
BAU_elec = User.Contracts.TOUEgoistContract.TOUEgoistContract("BAU_elec", elec, TOU_prices)
world.register_contract(BAU_elec)

flat_prices_heat = "flat_prices_heat"
BAU_heat = User.Contracts.FlatEgoistContract.FlatEgoistContract("BAU_heat", heat, flat_prices_heat)
world.register_contract(BAU_heat)

owned_by_aggregator = "owned_by_aggregator"
cooperative_contract_elec = User.Contracts.FlatCooperativeContract.FlatCooperativeContract("cooperative_contract_elec", elec, owned_by_aggregator)
world.register_contract(cooperative_contract_elec)

cooperative_contract_heat = User.Contracts.FlatCooperativeContract.FlatCooperativeContract("cooperative_contract_heat", heat, flat_prices_heat)
...
wind_turbine = User.Devices.NonControllableDevice.WindTurbine.WindTurbine("wind_turbine", cooperative_contract_elec, WT_producer, aggregator_elec, "ECOS", "ECOS")  # creation of a wind turbine
world.register_device(wind_turbine)  # registration of a production device

heat_production = User.Devices.NonControllableDevice.GenericProducer.GenericProducer("heat_production", cooperative_contract_heat, DHN_producer, aggregator_heat, "ECOS", "ECOS")  # creation of a heat production unit
...
# # # BAU contracts
world.agent_generation(10, "usr/AgentTemplates/ECOS2020/AgentECOS_1_BAU.json", [aggregator_elec, aggregator_heat], {"LVE": TOU_prices, "Heat": flat_prices_heat})
# world.agent_generation(10, "usr/AgentTemplates/ECOS2020/AgentECOS_2_BAU.json", [aggregator_elec, aggregator_heat])
# world.agent_generation(5, "usr/AgentTemplates/ECOS2020/AgentECOS_5_BAU.json", [aggregator_elec, aggregator_heat])
#
# # DLC contracts
# world.agent_generation(10, "usr/AgentTemplates/ECOS2020/AgentECOS_1_DLC.json", [aggregator_elec, aggregator_heat])
# world.agent_generation(10, "usr/AgentTemplates/ECOS2020/AgentECOS_2_DLC.json", [aggregator_elec, aggregator_heat])
# world.agent_generation(10, "usr/AgentTemplates/ECOS2020/AgentECOS_5_DLC.json", [aggregator_elec, aggregator_heat])
#
# # Curtailment contracts
# world.agent_generation(10, "usr/AgentTemplates/ECOS2020/AgentECOS_1_curtailment.json", [aggregator_elec, aggregator_heat])
# world.agent_generation(10, "usr/AgentTemplates/ECOS2020/AgentECOS_2_curtailment.json", [aggregator_elec, aggregator_heat])
# world.agent_generation(10, "usr/AgentTemplates/ECOS2020/AgentECOS_5_curtailment.json", [aggregator_elec, aggregator_heat])
...
price_manager_owned_by_the_aggregator = User.Daemons.PriceManagerDaemon.PriceManagerDaemon("toto", 1, {"nature": elec.name, "buying_price": 0, "selling_price": 0, "identifier": owned_by_aggregator})  # as these devices are owned by the aggregator, energy is free
price_manager_heat = User.Daemons.PriceManagerDaemon.PriceManagerDaemon("Picsou", 1, {"nature": heat.name, "buying_price": 0.15, "selling_price": 0.1, "identifier": flat_prices_heat})  # sets prices for flat rate
price_manager_elec = User.Daemons.PriceManagerTOUDaemon.PriceManagerTOUDaemon("Flairsou", 1, {"nature": elec.name, "buying_price": [0.2125, 0.15], "selling_price": [0, 0], "hours": [[6, 12], [14, 23]], "identifier": TOU_prices})  # sets prices for TOU rate
...
aggregator_balances = User.Dataloggers.Balances.AggregatorBalanceDatalogger()
...
# ECOS_aggregator_datalogger = User.Dataloggers.ECOSDatalogger.ECOSClusterDatalogger()
